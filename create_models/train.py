...
saving_directory = "/scratch/cpandey1/results_vgg16"
...
def dataloading():
    ...
    imbalance_ratio = 1.5*(2*len(ori_nf)) / (4*len(ori_fl))
    ...
    train_loader = DataLoader(dataset=train_set, batch_size=opt.batch_size, num_workers=8, pin_memory = True, shuffle = True)

    val_loader = DataLoader(dataset=val_set, batch_size=opt.batch_size, num_workers=8, pin_memory = True, shuffle = False)
...
def train():
    ...
    net = VGG16(ipt_size=(512, 512), pretrained=True).to(device) #default: training mode is True. For prediction prob, set train=False 
    ...
    for epoch in range(1, num_epochs):
        ...
        if val_tss>best_tss:
            ...
            best_path = f'{model_dir}/Epoch_{epoch}_fold{opt.fold}.pth'
            torch.save({
    ...
    PATH = f'{model_dir}/fold{opt.fold}.pth'
    torch.save({
                'epoch': num_epochs,
                'model_state_dict': model.module.state_dict(),
                'optimizer_state_dict': optimizer.state_dict()
    ...
    df.to_csv(f'{results_dir}/fold{opt.fold}.csv', index=False, header=True)
